...
# ── 이준희 리뷰 메모 ──────────────────────────────────────────────────────────
# 노트북(.ipynb)을 원본은 그대로 두고 셀 단위로 파싱해서 자기만의 실행기
# (executor, 익제큐터 — 코드를 실제로 돌리는 부분)로 돌리는 구조가 재미있음.
# nbconvert 같은 표준 도구 대신 직접 구현한 이유가, Colab 전용 셀(!apt-get 등)을
# 골라서 걸러내고 fig.show()로 GUI 창이 뜨는 것도 막아야 했기 때문인 것 같은데,
# 그 필요성이 remove_colab_font_setup / show() 정규식 제거 코드에 잘 드러남.
# ────────────────────────────────────────────────────────────────────
"""
...
def remove_colab_font_setup(source: str) -> str:
    ...
    # 이준희 리뷰: 셀 전체를 스킵하지 않고 문제되는 줄만 골라서 지우는 방식이라
    # 같은 셀 안에 있는 다른 필요한 코드(데이터 로드 등)를 살릴 수 있는 게 장점.
    lines = []
...
def run_notebook(source_path: Path, output_prefix: str) -> None:
    ...
    for index, cell in enumerate(notebook.get("cells", [])):
        ...
        try:
            ...
            # 이준희 리뷰: 셀 인덱스를 예외 메시지에 넣어준 게 실전에서 큰 도움이 될 듯.
            # 노트북이 20개 셀 가까이 되는데 "어디서 실패했는지" 바로 알 수 있음.
            raise RuntimeError(
