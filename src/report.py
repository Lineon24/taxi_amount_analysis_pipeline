...
# ── 리뷰 메모 ──────────────────────────────────────────────────────────
# 이 파일은 계산을 다시 안 하고 이미 저장된 산출물(JSON/CSV/PNG)만 읽어서
# 조립만 한다는 원칙이 명확함. 그 덕분에 리포트 문구만 고칠 때 전체 파이프라인을
# 다시 안 돌려도 되는 게 실용적. 다만 이 말은 곧 "다른 스크립트가 파일 이름이나
# 컬럼을 바꾸면 이 파일이 조용히 깨진다"는 뜻이기도 해서, load_json에서 필수
# 키를 검증하는 부분이 왜 있는지 이해가 됨(빨리 실패하게 만드는 것).
# ────────────────────────────────────────────────────────────────────
"""
