...
# ── 리뷰 메모 ──────────────────────────────────────────────────────────
# 이 파일에서 제일 눈에 띄는 건 IQR(사분위범위, Interquartile Range) 경계를
# train에서만 계산해서 test에도 그대로 적용하는 부분(build_bounds/apply_bounds).
# 전처리 단계에서부터 데이터 누수(data leakage, 데이터 리키지 — 미래/정답 정보가
# 학습에 섞여 들어가는 문제)를 신경 쓴 게 잘 보임. 보통 이 단계는 대충 넘어가고
# 모델링에서만 조심하는 경우가 많은데 여기선 처음부터 일관되게 적용함.
# ────────────────────────────────────────────────────────────────────
"""
...
TRAIN_END_DAY = 20
IQR_K = 1.5
# 이준희 리뷰: RatecodeID 1.0(일반 요금)만 k=3.0으로 더 느슨하게 잡은 이유가 궁금함.
# 일반 요금이 표본 수가 압도적으로 많아서 표준 1.5 배수로는 정상 장거리
# 운행까지 이상치로 잘라낼 위험이 있다고 판단한 것으로 추정됨 — 근거를 주석으로
# 한 줄 남겨두면 나중에 다른 사람이 볼 때 더 좋을 듯.
...
def split_by_date(frame: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # 이준희 리뷰: 무작위 분할(random split)이 아니라 날짜 기준으로 자른 것이 핵심.
    # 요금 예측처럼 시간에 따라 패턴이 바뀔 수 있는 문제에서 무작위 분할을 쓰면
    # 미래 정보가 학습에 섞여 실제보다 성능이 좋게 나오는 착시가 생길 수 있음.
    may = frame.loc[
...
def clean_common(frame: pd.DataFrame, passenger_mode: float) -> pd.DataFrame:
    ...
    # 이준희 리뷰: passenger_mode(최빈값)를 train에서만 구해서 인자로 넘기고 test에도
    # 그대로 쓰는 부분 — 위의 IQR 경계와 같은 원칙(train 통계만 학습, test엔 적용만)
    # 을 일관되게 지키고 있어서 함수 하나만 봐도 전체 설계 철학이 읽힘.
...
def build_bounds(train: pd.DataFrame) -> pd.DataFrame:
    ...
    # 이준희 리뷰: 이상치 기준을 전체 데이터가 아니라 RatecodeID(요금제)별로 따로 잡은 게
    # 좋은 판단이라고 생각함. 공항 정액 요금제와 일반 요금제는 거리·속도 분포 자체가
    # 다른데 하나의 기준으로 자르면 한쪽이 과하게 잘려나갈 수 있음.
    records = []
...
def save_outputs(train: pd.DataFrame, test: pd.DataFrame) -> None:
    ...
    # 이준희 리뷰: ID 포함/미포함 버전을 둘 다 저장하는 이유는 모델링 쪽(modeling.py)에서
    # PULocationID/DOLocationID를 타깃 인코딩용 그룹 키로 쓰기 때문인 듯.
    # 파일 하나로 통일하지 않고 용도별로 나눠 저장한 게 두 스크립트 사이의
    # 의존관계를 명확히 해주는 것 같음.
