...
# ── 이준희 리뷰 메모 ──────────────────────────────────────────────────────────
# 다운로드 → 로딩 → 비교 → 저장까지 하나의 흐름으로 딱 떨어져서 읽기 편함.
# 특히 Pandas와 Polars 결과가 안 맞으면 main()에서 바로 예외를 던지게 한 게 마음에 듦
# ────────────────────────────────────────────────────────────────────
"""
...
def download_file(url: str, destination: Path, force: bool = False) -> bool:
    ...
    # 이준희 리뷰: .download 임시파일에 먼저 쓰고 os.replace로 교체하는 패턴 좋음.
    # 다운로드 도중 죽어도 기존 원본 파일이 반쪽짜리로 덮어써지지 않음.
...
def load_with_polars(path: Path) -> tuple[pl.DataFrame, dict]:
    ...
    # 이준희 리뷰: Pandas 쪽과 완전히 같은 키 구조로 result dict를 만들어서
    # compare_results()에서 그냥 나란히 비교할 수 있게 한 설계가 깔끔함.
    started = time.perf_counter()
    try:
...
def compare_results(pandas_result: dict, polars_result: dict) -> dict:
    ...
    # 이준희 리뷰: sum 비교에 np.isclose(부동소수점 오차 허용 비교)를 쓴 점 좋음.
    # 두 라이브러리가 합산 순서/내부 자료형이 달라 완전히 같은 값이 안 나올 수 있는데
    # 그걸 미리 감안한 것으로 보임.
...
def main() -> None:
    ...
    del pandas_frame
    gc.collect()
    # 이준희 리뷰: 두 프레임을 동시에 메모리에 안 들고 하나씩 del + gc.collect() 하는 게
    # 대용량 Parquet 특성상 합리적인 선택 같음. 다만 gc.collect() 호출 비용이 있어서
    # 데이터가 작으면 오히려 손해일 수도 있을 듯 (지금 케이스엔 문제 없어 보임).
