...
# ── 이준희 설계 노트(내가 남기는 회고) ─────────────────────────────────────────
# 가장 공을 들였던 파생 변수 설계
#  1) 타깃 인코딩(target encoding, 타깃 인코딩 — 범주형 값을 그 값이 속한 그룹의
#     타깃 평균 등으로 바꾸는 인코딩 방식)을 K-Fold OOF(out-of-fold, 아웃오브폴드
#     — 자신이 속한 폴드가 아닌 나머지 폴드 통계로 값을 채우는 방식)로 계산해서
#     누수를 막았다. 노선 조합이 19,000개나 돼서 타깃 인코딩 없이는
#     PULocationID/DOLocationID를 제대로 쓸 수가 없는데, 그냥 전체 평균으로
#     인코딩하면 바로 과적합(overfitting, 오버피팅)으로 이어지길래 여기에 제일
#     공을 들였다.
# 덕분에 안정적으로 이전 결과를 과적합 없이 학습 시킬 수 있었다.
# 덕분에 성유정님이 만든 코드에서 더 좋은 성능을 끌어 올릴 수 있어 기쁘다.
# ────────────────────────────────────────────────────────────────────
"""
...
class StatisticalEncoder(BaseEstimator, TransformerMixin):
    ...
    # 이준희 설계 노트(가장 오래 고민한 부분): 처음엔 "과거 총 결제금액(total_amount)에서
    # 노선별/시간대별 평균·중앙값을 뽑아 피처로 쓰면 좋겠다"는 나의 아이디어였는데,
    # 그대로 하면 각 행이 자기 자신의 정답을 이미 반영한 평균값을 피처로 받는
    # 셈이라 100% 데이터 누수였다. "정답 자체를 그대로 가져오지 않으면서
    # 평균·중앙값·최댓값 같은 요약 통계는 쓰고 싶다"는 게 풀고 싶었던 문제였고,
    # 내가 생각한 답은 "내 행을 통계 계산에서 빼면 된다"였다. 그걸 전체 데이터에 대해
    # 한 행씩 다 빼면서 계산하면 너무 느리니, K-Fold로 미리 그룹을 나눠서
    # "이 fold에 속한 행들은 나머지 fold의 통계만 본다"로 근사한 게 지금의
    # OOF(out-of-fold) 방식이다. _oof() 함수가 그 핵심 구현.
    ...
    def _smooth(self, stat: pd.DataFrame, prior: float) -> pd.Series:
        ...
        """
        # 이준희 설계 노트: 베이지안 평활화(Bayesian smoothing) 방식으로 짰다. smoothing
        # 값(m=20)이 "가상의 사전 관측치 수"처럼 작동해서, count가 20보다
        # 훨씬 작은 그룹은 prior 쪽으로, 훨씬 큰 그룹은 자기 평균 쪽으로
        # 자연스럽게 수렴하게 만드는 게 목적. 20이라는 숫자 자체는 아직 정식으로
        # 검증 성능 기준 스윕(sweep, 값을 여러 개 바꿔가며 테스트)을 해본 건 아니라
        # 다음에 시간 나면 다른 값들이랑 비교해볼 만한 지점.
        m = self.smoothing
    ...
    def _oof(self, keys, values, prior, use_median=False) -> np.ndarray:
        ...
        # 이준희 설계 노트: 이 함수가 "행을 제외시키고 평균/중앙값을 계산한다"는 아이디어의
        # 실제 구현이다. splitter.split()으로 나눈 fold마다, valid_idx(검증 쪽)
        # 행들은 반드시 train_idx(나머지 쪽) 통계로만 값을 채운다 — 자기가 속한
        # fold의 통계는 절대 자기 자신에게 쓰이지 않는다. use_median=True를 넘기면
        # 평균 대신 중앙값으로도 같은 방식이 되게 만들어서, 이후에 최댓값 등
        # 다른 요약 통계로 확장하고 싶을 때도 이 구조를 그대로 재사용할 수 있게
        # 짰다.
    ...
    def fit_transform(self, X: pd.DataFrame, y=None, **kwargs) -> pd.DataFrame:
        ...
        # 이준희 설계 노트: sklearn의 fit_transform을 fit()+transform() 조합으로 안 쓰고
        # 완전히 따로 오버라이드했다. train은 OOF로, test는 fit에서 학습한
        # 통계를 그대로 적용해야 해서 두 경로가 근본적으로 다르기 때문이다.
...
def evaluate(y_true: np.ndarray, prediction: np.ndarray) -> dict:
    """회귀 평가 지표를 계산한다."""
    # 이준희 설계 노트: MAE/RMSE/R2 같은 표준 지표만으로는 부족한 것 같아서
    # within_5_dollars_percent(오차 5달러 이내 비율)를 추가했다. "MAE가 2.3이다"
    # 보다 "예측의 몇 %가 실제 요금과 5달러 이내로 맞았다"가 리포트 볼 사람
    # 입장에서 훨씬 와닿을 것 같아서 넣은 지표.
...
def report_importance(pipeline: Pipeline, sample_X: pd.DataFrame) -> pd.DataFrame:
    ...
    # 이준희 설계 노트: gain 기준 중요도의 한계(다중공선성 있는 변수끼리 중요도가 임의로
    # 나뉠 수 있다는 점)를 알고 있어서, 개별 피처가 아니라 그룹(타깃인코딩/
    # 빈도인코딩/시간/플래그/원본) 단위로 묶어서 보게 만들었다.
...
def main() -> None:
    ...
    # 이준희 설계 노트: 그냥 평균값만 찍는 베이스라인(baseline)과 나란히 비교하려고
    # 넣은 부분. 모델 MAE가 아무리 낮아도 베이스라인과 별 차이 없으면 그동안
    # 공들인 타깃 인코딩/조기 종료가 다 헛수고인 셈이라, 이 비교를 빼면
    # 안 된다고 생각해서 결과표에 항상 같이 찍히게 했다.
